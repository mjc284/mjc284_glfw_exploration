...
int main(void)
{
  ...
  window = glfwCreateWindow(900, 900, "Orbitng Planets", NULL, NULL);
  if (!window)
  {
    glfwTerminate();
    return -1;
  ...
  for(int i = 0; i < n_planets; i++)
  {
    ...
    distance = mag(planets[i].pos, 2);
    orbital_velocity = sqrt(const_G * mass_sun / distance);// / var2;
    planets[i].vel[0] = orbital_velocity * -1 * planets[i].pos[1]/distance;
    planets[i].vel[1] = orbital_velocity * 1 * planets[i].pos[0]/distance;
  }

  //Set Sun's mass and radius.
  planets[0].pos[0] = 0;
  planets[0].pos[1] = 0;
  planets[0].radius = 0.02;
